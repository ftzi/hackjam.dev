...
import { ArrowLeft, Award, Calendar, CalendarDays, Clock, Edit, Globe, MapPin, Trash2, Users } from "lucide-react"
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        <div className="flex items-center space-x-2 bg-muted p-2 rounded-md">
          <Switch id="creator-mode" checked={isCreator} onCheckedChange={setIsCreator} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{formatDate(event.registrationDeadline)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {event.registeredTeams}/{event.teamLimit} teams registered
                    </p>
                  </div>
                </div>
              </div>
            ...
            <TabsContent value="rules" className="space-y-4 pt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Important guidelines and requirements for participation</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <ul className="list-disc pl-5 text-sm text-muted-foreground">
                        <li>Maximum {event.participantsPerTeam} participants per team</li>
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="list-disc pl-5 text-sm text-muted-foreground">
                        <li>Maximum {event.teamLimit} teams</li>
                        <li>Maximum {event.maxParticipants} total participants</li>
                        <li>Registration closes on {formatDate(event.registrationDeadline)}</li>
                      </ul>
                    </div>
                  </div>
                </CardContent>
            ...
            <TabsContent value="schedule" className="space-y-4 pt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Detailed timeline of activities during the hackathon</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    {event.schedule.map((item, index) => (
                      <div key={index} className="flex">
                        <div className="mr-4 flex flex-col items-center">
                          ...
                          {index < event.schedule.length - 1 && <div className="h-full w-px bg-border" />}
                        </div>
            ...
            <TabsContent value="prizes" className="space-y-4 pt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Rewards for the winning teams</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-6 md:grid-cols-3">
                    {event.prizes.map((prize, index) => (
                      <Card key={index} className="overflow-hidden border-0 bg-muted">
                        <CardHeader className="pb-3">
                          <div className="flex items-center gap-2">
                            <Award className="h-5 w-5 text-primary" />
                            <CardTitle className="text-lg">{prize.place} Place</CardTitle>
                          </div>
                        </CardHeader>
                        <CardContent>
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {event.registeredTeams}/{event.teamLimit}
                  </span>
                </div>
                <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                  <div
                    className="h-full bg-primary rounded-full"
                    style={{ width: `${(event.registeredTeams / event.teamLimit) * 100}%` }}
                  />
                </div>
                ...
                <div className="flex justify-between pt-2">
                  ...
                  <span className="font-medium">
                    {event.registeredTeams * event.participantsPerTeam}/{event.maxParticipants}
                  </span>
                </div>
                <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                  <div
                    className="h-full bg-primary rounded-full"
                    style={{
                      width: `${((event.registeredTeams * event.participantsPerTeam) / event.maxParticipants) * 100}%`,
                    }}
                  />
                </div>
